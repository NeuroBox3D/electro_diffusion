...
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_ions(const std::vector<std::string>& vIons)
{
	 m_nIon = vIons.size();
	 _PHI_ = m_nIon;
...
// set ion valencies
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_valencies(const std::vector<int>& vValencies)
{
...
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_reversal_potentials(const std::vector<number>& vRevPot)
{
...
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_specific_conductances(const std::vector<number>& vSpecCond)
{
...
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_specific_capacities(const std::vector<number>& vSpecCap)
{
...
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_diffusion_constants(const std::vector<number>& vDiff)
{
...
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_permettivities(const number eps_dend, const number eps_mem)
{
	// make sure all entries are 0.0
...
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_membrane_thickness(const number d)
{
	m_mem_thickness = d;
}
...
// set constant dendritic radius
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_dendritic_radius(const number r)
{
...
// set constant dendritic radius
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_rtf(number R, number T, number F)
{
	m_R = R;
	m_T = T;
	m_F = F;
...
template <typename TDomain>
void PNP1D_FV1<TDomain>::set_represented_dimension(size_t dim)
{
	switch (dim)
	{
...
template<typename TDomain>
void PNP1D_FV1<TDomain>::prepare_setting(const std::vector<LFEID>& vLfeID, bool bNonRegularGrid)
{
...
template<typename TDomain>
bool PNP1D_FV1<TDomain>::use_hanging() const
{
	return true;
}
...
void PNP1D_FV1<TDomain>::prep_elem_loop(const ReferenceObjectID roid, const int si)
{}

//	prepares one element for assembling
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::prep_elem(const LocalVector& u, GridObject* elem, const ReferenceObjectID roid, const MathVector<worldDim> vCornerCoords[])
{
...
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::fsh_elem_loop()
{}

// virtual Assembling of Defect (Stiffness part)
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::add_def_A_elem(LocalVector& d, const LocalVector& u, GridObject* elem, const MathVector<worldDim> vCornerCoords[])
{
...
// volume parts (using lumping)
	...
	// potential equation
		...
		*/
		...
		for (size_t i = 0; i < m_nIon; i++)
		{
			d(_PHI_, co) += u(i, co) * m_vValency[i] * m_F * scvVolume;
...
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::add_def_M_elem(LocalVector& d, const LocalVector& u, GridObject* elem, const MathVector<worldDim> vCornerCoords[])
{
	// get finite volume geometry
	static const TFVGeom& fvgeom = GeomProvider<TFVGeom>::get();
...
template<typename TDomain>
template<typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::add_rhs_elem(LocalVector& rhs, GridObject* elem, const MathVector<worldDim> vCornerCoords[])
{}

// Assembling of Jacobian (Stiffness part)
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::add_jac_A_elem(LocalMatrix& J, const LocalVector& u, GridObject* elem, const MathVector<worldDim> vCornerCoords[])
{
	// get finite volume geometry
	static const TFVGeom& fvgeom = GeomProvider<TFVGeom>::get();
	...
	// ion concentrations
		...
		// electricity
			...
			// deriv wrt concs
				...
				// scale by concentration and constants
				el_flux *= scvfArea * m_vValency[i] * m_F/(m_R*m_T) * scvf.shape(sh);
...
// volume parts (using lumping)
	...
	// potential equation
		...
		*/
		// charge density term
		for (size_t i = 0; i < m_nIon; i++)
			J(_PHI_, co, i, co) += m_vValency[i] * m_F * scvVolume;
...
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::add_jac_M_elem(LocalMatrix& J, const LocalVector& u, GridObject* elem, const MathVector<worldDim> vCornerCoords[])
{
	// get finite volume geometry
	static const TFVGeom& fvgeom = GeomProvider<TFVGeom>::get();
...
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::prep_err_est_elem_loop(const ReferenceObjectID roid, const int si)
{
...
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::prep_err_est_elem(const LocalVector& u, GridObject* elem, const MathVector<worldDim> vCornerCoords[])
{
...
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::compute_err_est_A_elem(const LocalVector& u, GridObject* elem, const MathVector<worldDim> vCornerCoords[], const number& scale)
{
...
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::compute_err_est_M_elem(const LocalVector& u, GridObject* elem, const MathVector<worldDim> vCornerCoords[], const number& scale)
{
...
template<typename TDomain>
template <typename TElem, typename TFVGeom>
void PNP1D_FV1<TDomain>::fsh_err_est_elem_loop()
{
...
#ifdef UG_DIM_1
	template class PNP1D_FV1<Domain1d>;
#endif
#ifdef UG_DIM_2
	template class PNP1D_FV1<Domain2d>;
#endif
#ifdef UG_DIM_3
	template class PNP1D_FV1<Domain3d>;
#endif
